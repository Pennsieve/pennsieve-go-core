...
func (q *Queries) AddPackages(ctx context.Context, records []models.PackageParams) ([]models.Package, error) {
	// Steps:
	...
	parentIdMap := map[int64][]models.PackageParams{}
	...
	// format all vals at once
	var allInsertedPackages []models.Package
	...
	for rows.Next() {
		var currentRecord models.Package
		err = rows.Scan(
			&currentRecord.Id,
			&currentRecord.Name,
...
func (q *Queries) GetPackageChildren(ctx context.Context, parent *models.Package, datasetId int, onlyFolders bool) ([]models.Package, error) {
	folderFilter := ""
	if onlyFolders {
...
func checkUpdateName(item *models.PackageParams, index int, newName string, namesInFolder []string) {
